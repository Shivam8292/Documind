  ...
  return (
    <div className={`min-h-screen transition-all duration-300 ${d ? "bg-gray-950" : "bg-gradient-to-br from-slate-100 via-indigo-50 to-purple-100"} flex flex-col items-center px-4 py-8 relative`}>
      ...
      <div className="w-full max-w-2xl flex items-center justify-between mb-8">
        <div>
          <h1 className={`text-5xl font-black tracking-tighter ${d ? "text-white" : "text-indigo-600"}`}>
            📄 DocuMind
          </h1>
          <p className={`text-sm mt-1 font-medium ${d ? "text-gray-400" : "text-gray-400"}`}>
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => setDark(!d)}
            className={`px-4 py-2 rounded-full text-sm font-bold transition cursor-pointer shadow-md ${d ? "bg-gray-800 text-yellow-400 hover:bg-gray-700" : "bg-white text-gray-600 hover:bg-gray-50 border border-gray-100"}`}
          >
            {d ? "☀️" : "🌙"}
          </button>
        </div>
      ...
      <div className={`w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden transition-all duration-300 ${d ? "bg-gray-900 border border-gray-800" : "bg-white/80 backdrop-blur-md border border-white"}`}>
        ...
        {/* Upload Section */}
        <div className={`p-5 border-b transition-colors duration-300 ${d ? "bg-gray-800/80 border-gray-700" : "bg-gradient-to-r from-indigo-50 to-purple-50 border-gray-100"}`}>
          <div className="flex flex-col sm:flex-row items-center gap-3">
            ...
            <button
              ...
              className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:from-indigo-300 disabled:to-purple-300 text-white font-black px-6 py-3 rounded-2xl transition cursor-pointer shadow-lg shadow-indigo-200 text-sm"
            >
        ...
        <div className="h-96 overflow-y-auto p-6 flex flex-col gap-4">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
              {msg.type === "system" ? (
                <div className={`text-sm px-4 py-2 rounded-2xl w-full text-center font-medium ${d ? "bg-yellow-900/40 text-yellow-300 border border-yellow-700/40" : "bg-yellow-50 text-yellow-700 border border-yellow-200"}`}>
                  {msg.text}
                </div>
              ) : msg.type === "user" ? (
                ...
                <div className={`px-4 py-3 rounded-2xl rounded-tl-sm max-w-xs lg:max-w-md text-sm shadow-sm ${d ? "bg-gray-800 text-gray-100 border border-gray-700" : "bg-gray-50 text-gray-800 border border-gray-100"}`}>
                  ...
                  {msg.sources && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {msg.sources.map((s, j) => (
                        <span key={j} className={`text-xs px-2 py-1 rounded-full border font-medium ${d ? "bg-gray-700 text-gray-300 border-gray-600" : "bg-white text-indigo-500 border-indigo-100"}`}>
                          📌 {s}
                        </span>
                      ))}
                    </div>
        ...
        {/* Input */}
        <div className={`p-4 border-t flex gap-3 transition-colors ${d ? "border-gray-700 bg-gray-900" : "border-gray-100 bg-gray-50/50"}`}>
          <input
            type="text"
